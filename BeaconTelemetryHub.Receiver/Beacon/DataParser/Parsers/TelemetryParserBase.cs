...
namespace BeaconTelemetryHub.Receiver.Beacon.DataParser.Parsers
{
    public abstract class TelemetryParserBase
    {
        ...
        private const int ExpectedServiceDataLengthForBeaconIdentifier = 9;
        private const int ExpectedServiceDataLengthForBeaconProtocolVersion = 1;
        public const int ExpectedServiceDataLengthForTelemetryType = 10;
        protected static string GetBeaconIdentifier(ReadOnlySpan<byte> rawServiceData)
        {
            if (rawServiceData.Length < ExpectedServiceDataLengthForBeaconIdentifier)
            {
        ...
        {
            if (rawServiceData.Length < ExpectedServiceDataLengthForBeaconProtocolVersion)
            {
